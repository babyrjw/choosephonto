...
public class PhotoSelectorActivity extends Activity implements onItemClickListener, onPhotoItemCheckedListener, OnItemClickListener, OnClickListener {
	...
	public static final int SEND_CAMERA = 0;
    ...
    private ArrayList<PhotoModel> cameras;
...
//		ImageLoader.getInstance().init(config);
        ...
        cameras = new ArrayList<PhotoModel>();
	...
	private void catchPicture() {
		...
		CommonUtils.launchActivityForResult(this, captureIntent, REQUEST_CAMERA);
	}

	@Override
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == REQUEST_CAMERA && resultCode == RESULT_OK) {
			PhotoModel photoModel = new PhotoModel(mCapturedPhotoUri.getPath());
			// selected.clear();
			// //--keep all
			// selected photos
			// tvNumber.setText("(0)");
			...
			// ///////////////////////////////////////////////////////////////////////////////////////////
            cameras.clear();
            cameras.add(photoModel);
            Bundle bundle = new Bundle();
            bundle.putSerializable("photos", cameras);
            Intent intent = new Intent(this, CameraPreviewActivity.class);
            intent.putExtras(bundle);
            intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
            CommonUtils.launchActivityForResult(this, intent, SEND_CAMERA);
		}
        else if (requestCode == SEND_CAMERA && resultCode == RESULT_OK){
            ok(cameras);
        }
	}

	/** 完成 */
	private void ok(ArrayList<PhotoModel> list) {
		if (list.isEmpty()) {
